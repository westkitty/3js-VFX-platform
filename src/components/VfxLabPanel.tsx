...
export const VfxLabPanel: React.FC<VfxLabPanelProps> = (props) => (
  <div className="w-80 h-full bg-slate-950/90 border-l border-slate-800/80 p-4 flex flex-col gap-4 z-20 text-slate-200 overflow-y-auto backdrop-blur-md">
    <div className="flex items-center justify-between pb-3 border-b border-slate-800">
      ...
      <button onClick={props.onTriggerCast} className="flex items-center gap-1 px-2.5 py-1 rounded bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-medium shadow-md shadow-cyan-500/20"><Sparkles className="w-3 h-3" /><span>Cast Preview</span></button>
    </div>
    <VfxTimelineControls engine={props.engine} previewState={props.previewState} onSeekPreview={props.onSeekPreview} onRestartPreview={props.onRestartPreview} />
