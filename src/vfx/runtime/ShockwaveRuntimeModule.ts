...
export class ShockwaveRuntimeModule implements RuntimeVfxModule {
  public readonly type = 'shockwave' as const;
  private readonly ring: ShockRing;

  constructor(config: AbilityModuleConfig, context: VfxModuleFactoryContext) {
    this.ring = new ShockRing(context.scene, context.request.target, 1);
